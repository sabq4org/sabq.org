...
async function tick(trigger: string): Promise<void> {
  ...
  try {
    ...
    if (summary.previews || summary.reports || summary.arabRoundups || summary.stageReports || summary.errors) {
      console.log(
        `[WC News Job] (${trigger}) previews=${summary.previews} reports=${summary.reports} arabRoundups=${summary.arabRoundups} stageReports=${summary.stageReports} skipped=${summary.skipped} errors=${summary.errors}`
      );
    }
  } catch (error) {
