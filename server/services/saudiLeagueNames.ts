...
/** أسماء عربية معروفة للأندية السعودية — لمطابقة لوحات TheSports وغيرها. */
const SAUDI_CLUB_AR_NAMES: string[] = [
  ...new Set([...Object.values(SPL_TEAM_AR), ...Object.values(SPL_DIV1_TEAM_AR)]),
];

/**
 * جذور إنجليزية شائعة (بلا فواصل) لأندية روشن/يلو — TheSports غالبًا يعيد
 * الاسم اللاتيني في الودّيات بينما واجهتنا تُعرِّب من API-Football.
 */
const SAUDI_CLUB_EN_COMPACT: string[] = [
  "alahli", "alhilal", "alnassr", "alittihad", "alshabab", "alettifaq",
  "alfateh", "altaawoun", "altaawon", "alqadsiah", "alqadisiyah", "alfayha",
  "alkhaleej", "alokhdood", "alkholood", "alriyadh", "neom", "damac",
  "alnajma", "alhazm", "alwehda", "alraed", "alfaisaly", "albatin",
  "altai", "alorubah", "aloula", "abha",
];

function compactTeamKey(name: string): string {
  return (name || "")
    .toLowerCase()
    .replace(/[أإآا]/g, "ا")
    .replace(/ة/g, "ه")
...
/** هل الاسم يبدو لنادٍ سعودي (عربي أو إنجليزي)؟ لا يشمل المنتخب. */
export function looksLikeSaudiClubTeamName(name: string): boolean {
  const t = (name || "").trim();
  if (!t) return false;
  for (const ar of SAUDI_CLUB_AR_NAMES) {
    if (ar && t.includes(ar)) return true;
  }
  const compact = compactTeamKey(t);
  if (!compact) return false;
  return SAUDI_CLUB_EN_COMPACT.some((en) => compact.includes(en));
}

/** تطابق مرن لاسمي فريق (عبر السكربت أو الاشتراك النصّي). */
export function teamNamesLooselyMatch(a: string, b: string): boolean {
  const ca = compactTeamKey(a);
  const cb = compactTeamKey(b);
  if (!ca || !cb) return false;
  if (ca === cb) return true;
  if (ca.length >= 4 && cb.length >= 4 && (ca.includes(cb) || cb.includes(ca))) return true;
  if (looksLikeSaudiClubTeamName(a) && looksLikeSaudiClubTeamName(b)) {
    for (const en of SAUDI_CLUB_EN_COMPACT) {
      if (ca.includes(en) && cb.includes(en)) return true;
    }
    for (const ar of SAUDI_CLUB_AR_NAMES) {
      const car = compactTeamKey(ar);
      if (car.length >= 3 && ca.includes(car) && cb.includes(car)) return true;
    }
  }
  // توكنات لاتينية مشتركة (rio/ave ↔ ريو آفي عبر rio إن وُجد في الطرفين)
  const latA: string[] = a.toLowerCase().match(/[a-z]{3,}/g) ?? [];
  const latB: string[] = b.toLowerCase().match(/[a-z]{3,}/g) ?? [];
  if (latA.some((t) => latB.includes(t))) return true;
  return false;
}
