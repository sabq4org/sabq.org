  ...
  try {
    ...
    const response = await client.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `أنت خبير في التسويق عبر البريد الإلكتروني في صحيفة سبق، متخصص في كتابة عناوين جذابة باللغة العربية تحقق أعلى معدلات فتح.

مهمتك: إنشاء 5 عناوين مختلفة لنشرة إخبارية، كل عنوان بأسلوب مختلف.

معيار سبق (إلزامي): عربية فصحى سليمة 100% بلا أي خطأ إملائي أو نحوي، تشويق بلا تضليل أو clickbait، ولا معلومة غير واردة في الأخبار المرفقة.
  ...
  try {
    ...
    const response = await client.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `أنت محرر صحفي خبير في صحيفة سبق، متخصص في صياغة ملخصات إخبارية جذابة تثير فضول القارئ.

مهمتك: تحويل الخبر إلى ملخص قصير ومشوق يدفع القارئ للنقر لقراءة المزيد.

معيار سبق (إلزامي): عربية فصحى سليمة 100% بلا أي خطأ إملائي أو نحوي، تشويق بلا تضليل، حقائق من الخبر فقط دون إضافة أو مبالغة.
