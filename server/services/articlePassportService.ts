...
function computeBodyAiTier(params: {
  ...
  if (hasSummary || hasBullets || hasAiEdits) return "assisted";
  return "human";
}
...
function buildExplanation(opts: {
  ...
  const bodyAr = bodyTier === "human" ? "كُتب بشريًا"
    : bodyTier === "assisted" ? "ساعد الذكاء الاصطناعي في الملخص"
  ...
  const bodyEn = bodyTier === "human" ? "Written by a human"
    : bodyTier === "assisted" ? "AI helped with the summary"
  ...
  const bodyUr = bodyTier === "human" ? "انسان نے تحریر کیا"
    : bodyTier === "assisted" ? "خلاصے میں AI کی مدد لی گئی"
