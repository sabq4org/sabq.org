...
export function registerTaskRoutes(app: Express) {
  ...
  // POST /api/tasks/bulk-complete - Complete multiple tasks or all open tasks
  app.post("/api/tasks/bulk-complete", taskLimiter, requireAuth, requireAnyPermission('tasks.edit_any', 'tasks.edit_own'), async (req, res) => {
    try {
      const { taskIds, allOpen } = req.body;
      const userId = (req.user as any).id;
      const userPermissions = await storage.getUserPermissions(userId);
      ...
      if (Array.isArray(taskIds) && taskIds.length > 0) {
        ...
        const filters: any = { limit: 1000, offset: 0 };
        if (!userPermissions.includes('tasks.view_all')) {
          filters.userIdForOwn = userId;
        }
        const { tasks } = await storage.getTasks(filters);
        targetIds = tasks.filter(t => t.status !== 'completed').map(t => t.id);
      }

      if (targetIds.length === 0) {
        return res.json({ success: true, count: 0, message: 'لا توجد مهام لإكمالها' });
      }
      ...
      for (const id of targetIds) {
        ...
        await storage.logTaskActivity({
          ...
          changes: {
            ...
            newValue: 'completed',
            description: 'تم إتمام المهمة عبر إجراء جماعي',
          },
        });
        completedCount++;
      }

      res.json({ success: true, count: completedCount, message: `تم إتمام ${completedCount} مهمة بنجاح` });
    } catch (error: any) {
      console.error('Error in bulk complete tasks:', error);
      res.status(500).json({ error: 'فشل في إتمام المهام' });
    }
  });

  // POST /api/tasks/bulk-delete - Delete multiple tasks
  app.post("/api/tasks/bulk-delete", taskLimiter, requireAuth, requireAnyPermission('tasks.delete_any', 'tasks.delete_own'), async (req, res) => {
    try {
      ...
      if (!Array.isArray(taskIds) || taskIds.length === 0) {
        return res.status(400).json({ error: 'يجب تحديد المهام المراد حذفها' });
      }
      ...
      res.json({ success: true, count: deletedCount, message: `تم حذف ${deletedCount} مهمة بنجاح` });
    } catch (error: any) {
