...
## 1. الهوية البصرية الجديدة — «أبيض نظيف + أخضر مقتصد»

المرجع: تصميم صفحة كأس آسيا على الويب (`sabq.org`). المبدأ الذي اعتمده المالك بالتراكم:

> **أبيض نظيف + لون أخضر واحد لمسة + مسطّح بلا ظلال + حدود حادة فاتحة + بطاقات مدمجة + أعلام/صور لون المحتوى.**

كل الألوان مركزية في **`Services/SportsTheme.swift`** (enum `SpTheme`). أهم القيم:

| الرمز | القيمة | الاستخدام |
|---|---|---|
| `screenGradient` | رمادي فاتح جدًّا `#F6F7F8→#F2F3F5` | خلفية كل الشاشات |
| `card` / `surface` | أبيض ناصع | كل البطاقات |
| `cardStroke` | رمادي فاتح حادّ `#D5D9DF` (1px) | حدّ البطاقات (بديل الظلّ) |
| `cardShadow` | **`.clear`** | لا ظلال (مسطّح تمامًا) |
| `green` | زمردي `#17 6E52` | اللمسة الوحيدة (ترويسات/أزرار/أرقام) |
| `onDark` / `onDarkDim` / `onDarkFaint` | أسود/رمادي/رمادي فاتح | حبر النصّ |
...
- **قاعدة التحويل** (كتلة خضراء → بطاقة بيضاء): الخلفية `card`+`cardStroke`، كل نصّ أبيض → `onDark*`، الأزرار النشطة = أخضر مملوء/نصّ أبيض والخاملة = حدّ أخضر/نصّ أخضر، وحذف الظلّ الأخضر. **يُستثنى الدلالي** (الكرت الأصفر، تمييز فريقين في الرسوم البيانية = مضيف أخضر/ضيف رمادي).
