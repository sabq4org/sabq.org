...
## 1. الهوية البصرية الجديدة — «iOS grouped + أبيض نظيف + أحمر VARA مقتصد»

المرجع: تصميم صفحة كأس آسيا على الويب (`sabq.org`). المبدأ الذي اعتمده المالك بالتراكم:

> **خلفية رمادية محايدة + بلوكات بيضاء كبيرة + لون مظهر قابل للتغيير (الأحمر افتراضيًا) + مسطّح بلا ظلال + حدود خافتة جدًا + أعلام/صور لون المحتوى.**

كل الألوان مركزية في **`Services/SportsTheme.swift`** (enum `SpTheme`). أهم القيم:

| الرمز | القيمة | الاستخدام |
|---|---|---|
| `screenGradient` | رمادي iOS فاتح `#F4F4F8→#F1F2F6` | خلفية كل الشاشات |
| `card` / `surface` | أبيض ناصع | كل البطاقات |
| `cardStroke` | رمادي خافت `#E8E9EC` (1px) | فصل ناعم للبطاقات عند الحاجة |
| `cardShadow` | **`.clear`** | لا ظلال (مسطّح تمامًا) |
| `green` | لون التطبيق المختار (`#BB3734` افتراضيًا) | اللمسة النشطة (ترويسات/أزرار/أرقام) مع بقاء الاسم للتوافق |
| `onDark` / `onDarkDim` / `onDarkFaint` | أسود/رمادي محايد/رمادي فاتح | حبر النصّ |
| `gold` | ذهبي | الميداليات + الكرت الأصفر فقط (دلالي) |

- **المظهر مفروض فاتح**: `.preferredColorScheme(.light)` في `SabqSportsApp.swift`.
- **الخلفية المحيطة** (`SpAmbientBackground`) = `screenGradient` فقط — بلا توهّجات/نقشة.
- **قاعدة التحويل** (كتلة ملونة → بطاقة بيضاء): الخلفية `card`+`cardStroke`، الزوايا `cardRadius = 28`، كل نصّ أبيض → `onDark*`، الأزرار النشطة = لون التطبيق المختار مملوء/نصّ أبيض والخاملة = رمادي/نصّ محايد، وحذف الظلّ الأخضر. **يُستثنى الدلالي** (الكرت الأصفر، تمييز فريقين في الرسوم البيانية = مضيف ملوّن/ضيف رمادي).

كل الشاشات حُوّلت لهذه الهوية: الرئيسية، البطولات، المباشر، المجتمع، الحساب، مركز المباراة، النادي، اللاعب.
...
## 15. الشاشات التعريفية لأول تشغيل (`Screens/OnboardingView.swift`)

تجربة Onboarding خضراء غامرة تُعرض **مرّةً واحدة** عند أول تشغيل، تُبرز أهمّ المزايا وتُعرّف أن التطبيق **أحد منتجات صحيفة سبق الإلكترونية**:
- **خمس صفحات** (`OBPage`: `kind` brand/symbol + عنوان/وصف/لون مميّز): (١) علامة **VARA** بشعارها، (٢) **مركز المباراة الحيّ**، (٣) **كل المباريات مباشرة**، (٤) **كل شيء عن البطولة**، (٥) **تابع فريقك** (وهي صفحة الـCTA «ابدأ الآن»).
- **خلفية Aurora متحرّكة** (`OBAurora`): تدرّج أخضر عميق + ٣ كرات ضبابية (`blur 80`) تنجرف بحركة `repeatForever`، ولونها يتبدّل مع لون الصفحة الحالية.
- **بطل متوهّج عائم** (`OBHero`): هالة لونية + دائرتان زجاجيّتان؛ شعار VARA لصفحة العلامة، أو رمز SF لبقية الصفحات، بحركة طفو `repeatForever`.
- **انتقال الصفحات** عبر `TabView(.page)` (مؤشّر مخصّص: حبّة ممدودة للنشطة). اتجاه التمرير مُثبَّت LTR لثبات الترتيب، والنصّ عربيّ متوسّط. كل صفحة تظهر بحركة fade/slide عبر `shown` في `onAppear`.
- **أزرار**: «تخطّي» أعلى اليمين، «التالي»/«ابدأ الآن» (زرّ أبيض بظلّ) أسفل، وتذييل هوية سبق دائم. الإنهاء يستدعي `onFinish` مع لمسة هابتيّة.
- **الربط**: `RootTabView` يحمل `@AppStorage("ob_seen_v1")` و`@State showOnboarding`؛ يُعرض كـ`fullScreenCover` عند `onAppear` إن لم يُرَ بعد، ويُضبط العلم عند الإنهاء. `preferredColorScheme(.dark)` لشريط حالة فاتح فوق الخلفية الخضراء.
- **تأجيل إذن الإشعارات**: في `AuthStore.loadUserData` لا يُطلب إذن الإشعارات (`enablePushNotifications`) إلا بعد ضبط `ob_seen_v1` — كي لا يتداخل نظام الإذن مع العرض التعريفي؛ يُطلب تلقائيًّا في الإقلاع التالي.
- **إضافة الملف لمشروع Xcode**: المشروع يستخدم مراجع ملفات صريحة (لا synchronized groups)، فأُضيف `OnboardingView.swift` يدويًّا في `project.pbxproj` (BuildFile `5B000013` / FileReference `5C000013` + مجموعة Screens + مرحلة Sources).
