...
- **Design-preview-before-build for UI work.** For substantial or design-forward UI changes (a new screen, an auth/login flow, a redesign of a view — anything where visual layout is itself a deliverable), first present a visual proposal and get approval BEFORE writing production code. Prefer a self-contained Artifact HTML mockup (so the design can be *seen*); at minimum give a concrete design plan (sections, layout, palette/type tokens). Make it faithful to the app's real design system (shadcn tokens + RTL Arabic on web; VARA green/gold identity on iOS), light and dark. Skip this step for trivial tweaks, refactors, bug fixes, backend-only changes, or when the owner says "just do it / اعتمد".
