...
Deploy configs:
- `Dockerfile` — Railway picks this over nixpacks. Runs `npm run build:server` (server-only bundle), defaults `SERVE_SPA=false`, copies `dist/` and `public/` (for `/branding/`). The image is intentionally headless; local docker-compose users wanting full-stack-on-one-port must override `SERVE_SPA=true` and supply a pre-built `dist/public/`.
- `railway.json` — sets `builder: DOCKERFILE` explicitly to match Railway's actual selection. Healthchecks `/health` with a 60-second timeout.
- `nixpacks.toml` — fallback only (kept for environments without Docker).
