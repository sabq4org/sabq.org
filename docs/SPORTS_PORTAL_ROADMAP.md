...
5. **القيمة السوقية للاعب — مُنفَّذة (قيمة التشكيلة، تحتاج تحقّق شكل على الإنتاج) 2026‑06‑23:** `getTsTeamPlayers(teamUuid)` عبر **`player/with_stat/list?team_id=`** (تحليل متسامح للحقول) → `getWcSquadMarketValues` يبني خريطة `اسم مُطبَّع → {value, currency}` عبر الجسر → `getSquad` يُثري كل لاعب بـ`marketValue`؛ تُعرض بالأخضر تحت اسم اللاعب في `WorldCupTeam`. المطابقة بالاسم الإنجليزي الكامل ثم اسم العائلة (`normPlayerName`). **⚠️ لم يُتحقَّق شكل الاستجابة محليًّا** (عنوان dev مُزال) — أسماء الحقول (`market_value`/`value`…) تخمين متسامح؛ **تحقّق على الإنتاج:** افتح `/api/world-cup/squad/<teamId>` وتأكّد ظهور `marketValue`؛ إن كان `null` للجميع فاضبط أسماء الحقول في `getTsTeamPlayers`. **متبقٍّ:** سجلّ الانتقالات (`player/transfer/list` بـuuid اللاعب من `with_stat`) + عرضه في بطاقة اللاعب.
