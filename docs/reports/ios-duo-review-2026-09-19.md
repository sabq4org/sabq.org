...
- المؤجل بقرار المالك: تبديل الحاوية `Split ↔ Stack` نفسه (يُبقي على إعادة بناء الشجرة عند الطيّ وإن لم يعد يفقد المقال)، و`.sidebarAdaptable` الذي يتطلب iOS 18.
...
**ما بقي من A بقرار المالك:** تبديل نوع الحاوية نفسه ما زال يعيد بناء شجرة التبويب عند الطيّ (تضيع `@State` الداخلية: موضع التمرير، `.task` يعيد الجلب، الأوراق المفتوحة تُغلق). البديل `NavigationSplitView` دائمًا مع `columnVisibility` متغيّر يحتاج جولة محاكي للروابط العميقة وزر الرجوع على المطوي.
...
| الملف | التغيير |
|---|---|
| `Components/SabqNavigation.swift` | `homeLayoutDidChange`، `onLayoutChange`، (سابقًا: عتبة العرض، `.constant(.all)`، `HomeReaderRoot` كجذر للقارئ) |
...
1. قرار `NavigationSplitView` دائمًا بدل التبديل (A المتبقي) — ملف واحد + جولة محاكي للروابط العميقة.
2. لقطات للشبكات المتكيفة على القارئ العريض (C) وللأوراق `.medium` (F).
3. اختبار إيماءة الرجوع (E) على جهاز حقيقي.
