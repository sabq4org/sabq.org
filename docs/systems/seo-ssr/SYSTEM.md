...
- **hreflang AR↔EN (2026-07-30):** لا تُفترض نفس الـ slug بين العربية والإنجليزية. الترجمة تولّد slug جديداً (`nnhfqhq` ≠ `7fmewmq`). الربط عبر `en_articles.seoMetadata.sourceArticleId` فقط؛ وإلا يُحذف رابط اللغة الشقيقة بدل الإشارة لـ 404. IndexNow/Google Indexing بعد الترجمة يستهدف `/en/article/...` مع `locale=en`.
