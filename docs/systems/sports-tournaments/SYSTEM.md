...
- **صفحة النادي:** `getTeamProfile` خلف `spl:teamprofile:v3:*` (SWR، منذ 2026-07-27: طازج 15د + بائت ساعة — كانت 5د/10د فتُبرد أندية الذيل الطويل ويصطدم أول زائر بمهلة 3ث → 503) + اكتشاف الترتيب يبدأ بروشن ثم البقية (لا 4 جداول دفعة على البرود).
- **تسخين بطاقات اللاعبين:** `getSquad` يُشغّل `warmSquadPlayerCards` بعد كل جلب (طازج أو مجدَّد) — تسلسلي، سقف 26 لاعبًا، تباعد 200ms، أخطاء مبتلعة. من يتصفح التشكيلة ينقر لاعبيه فيصيب كاشًا دافئًا بدل 503 البطاقة الباردة. لا يشمل form/market (أقسام lazy تملأ كاشها بالخلفية). تشبّع طابور API-Football يوقف التسخين ذاتيًا (رفض فوري).
