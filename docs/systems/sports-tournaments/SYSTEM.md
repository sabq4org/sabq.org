...
## كاش وأولوية مزوّد الرياضة — 2026-07-19
- خدمات API-Football الأساسية (البوابة الموحّدة/كأس العالم/الخليج/آسيا) تستخدم `withSportsSWR`: ذاكرة العملية L1 + Redis L2 بنسختي fresh/stale؛ يبقى الكاش بعد deploy، وغياب Redis يتدهور تلقائيًا للذاكرة والمزوّد.
- طلب `/api/sports/*` تفاعلي ويَسبق التسخين والمهام الرياضية المجدولة في طابور API-Football المشترك.
- ميزانية المصدر للطلب التفاعلي 3 ثوانٍ؛ بعدها يُعاد آخر stale صالح ويستمر تحديث Redis في الخلفية.
- قياس المزوّد يفصل `queueMs` عن `providerMs` و`totalMs` في سجل `[Sports Provider]`.
- `GET /api/sports/competitions` يعيد لقطة Redis/الذاكرة أو القائمة الأساسية فورًا؛ إثراء ~35 بطولة يعمل في الخلفية.
- صفحة اللاعب تبدأ بـ `GET /api/sports/player/:id` ثم تحمّل `GET /api/sports/player/:id/extras` (التاريخ/الانتقالات/الإصابات)، ثم market/form بصورة مستقلة.
