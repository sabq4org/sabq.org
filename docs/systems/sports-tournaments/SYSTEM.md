...
- **كاش:** SWR في ذاكرة العملية `blocks:wc:numbers-report:v5` — طازج 15د + SWR 15د. الطلب الأول يحسب؛ اللاحق من الذاكرة. `?fresh=1` يعيد الحساب. الحقل `cache.source` = `cache` | `computed`.
