...
> آخر مراجعة: 2026-07-28 (روشن ويب: تبويب الجولات الـ٣٤ على /roshn) | المالك: sports
...
- **تبويب «الجولات» على `/roshn` (2026-07-28):** في `RslMatches` — شريط الـ٣٤ عبر `GET /api/sports/pro-league/rounds` + `/round?name=` (نفس عقد iOS و`SportsHub.RoundsView`). تبويبات مباشر/اليوم/قادمة/نتائج تبقى من `/matches` بسقف معاينة 54 مباراة (≈6 جولات) حتى لا تُقطع الجولة من المنتصف؛ الجدول الكامل من تبويب الجولات فقط.
...
- **عرض مركز المباراة:** استخدم `sheet(isPresented:)` مع اختيار صريح بعد النقر؛ `sheet(item:)` على مصفوفة المباريات تسبب في iOS 26 فتح أول مباراة تلقائيًا عند وصول البيانات.
