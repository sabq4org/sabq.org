...
- تبقى الحقول الاختيارية الأخرى كما هي. فحص الإنتاج أكد حد `background_color` الحالي (20 حرفًا)، بينما لا تُضيق `source_type` و`subtitle` و`playbook` الأوسع قبل فحص طول مستقل.
