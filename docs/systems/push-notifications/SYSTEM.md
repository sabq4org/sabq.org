...
- **iOS `parseSabqDeepLink`/`extractDeepLink` (2026-09-23):** كل slug/id/token في الرابط العميق (`sabq://`, Universal Link, حقل `deeplink` في userInfo) يُغربل بنمط صارم (`PushNotifications.swift`) قبل التنقل — لا تُزِل هذا التحقق عند إضافة مضيف/مسار جديد، أضف نمطه للـ whitelist بدل تخفيف الشرط. توقيع حمولة APNs (`apns_signed`) لإثبات المصدر الخادمي لا يزال غير منفَّذ — راجع `docs/iOS-TECH-DEBT.md`.
