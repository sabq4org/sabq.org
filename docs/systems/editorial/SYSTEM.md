...
> آخر مراجعة: 2026-07-28 (إشعارات التحرير للمستفيدين فقط؛ محرّر EN عبر dashboard API) | المالك: editorial
...
- **شريط تحذير الترخيص في اللوحة:** لمراسل/كاتب رأي بلا ترخيص ساري أو بترخيص منتهٍ أو يحتاج تصحيحاً يظهر أعلى المحتوى في `DashboardLayout` (`MEDIA_LICENSE_DASHBOARD_WARNING`)؛ الضغط يفتح نموذج الترخيص. المنطق مشترك عبر `useMediaLicenseGate`.
- **تعطيل إنشاء مقال/خبر:** نفس الجمهور (بلا ترخيص / منتهٍ / يحتاج تحديثاً) — تُعطَّل أزرار «إنشاء مقال/خبر» و«ابدأ الكتابة» في الشريط ومساحة الكاتب وأخبار المراسل ودليل الكاتب؛ فتح `/dashboard/articles/new` يعيد إلى نموذج الترخيص.
