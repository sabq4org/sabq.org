...
> آخر مراجعة: 2026-07-20 (ضغط تنسيق خلية الترخيص في جدول الكتّاب) | المالك: editorial
...
- **ترخيص كاتب الرأي على الحساب:** أعمدة `users.media_license_*` بما فيها `media_license_expires_at` (إلزامي عند الإدخال). المسارات `GET/POST /api/opinion-author/media-license` (رفع الملف عبر R2/S3 الخاص؛ الحقل `licenseExpiresAt` بصيغة `YYYY-MM-DD`). الاستجابة: `submitted` / `valid` / `expired` / `expiringSoon` / `expiresAt`. `expiringSoon` = ساري ويتبقّى ≤ ٦٥ يوماً (~شهرين) → تنبيه أحمر («جدّد الترخيص») في البطاقة والسايدبار وصفحة الكتّاب. بعد إرسال صالح تظهر رسالة شكر؛ إن انتهى أو بلا تاريخ انتهاء تُعاد الاستمارة. المهلة التنظيمية لتقديم الترخيص لأول مرة: `2026-07-31`.
- **إدارة الترخيص من صفحة الكتّاب:** `/dashboard/opinion-writers` خلية الترخيص مضغوطة: شارة + أيقونة ملف، ثم تاريخ قصير `حتى d/M/yyyy` ورقم الترخيص (بدون تاريخ الإرسال ولا سطر تحذير إضافي). الفلترة والبحث و`GET .../media-license-file` كما هي. ترتيب العرض: منتهٍ → جدّد → بدون → ساري (الأقرب انتهاءً).
