...
> آخر مراجعة: 2026-07-20 (أداء قائمة المراسلين + نجمة المفضلة) | المالك: editorial
...
- **إدارة الترخيص — كتّاب الرأي:** `/dashboard/opinion-writers` خلية مضغوطة + فلترة؛ `GET /api/admin/opinion-writers/:id/media-license-file`.
- **إدارة المراسلين:** `/dashboard/reporters` (أدوار admin/editor/system_admin، صلاحية `users.view` أو `articles.view`). أعمدة: ترخيص، مدينة، منشورة، بانتظار المراجعة، آخر خبر، آخر دخول، مشاهدات — بلا يوم نشر. API: `GET /api/admin/reporters` + مقالات + ملف الترخيص. ترتيب: منتهٍ → جدّد → بدون → ساري. العنوان عبر `DashboardPageHeader` (نجمة المفضلة). `listReporters` يجمّع الإحصاءات وآخر خبر في Postgres (`CASE` للمالك: reporter→author→submitter + `DISTINCT ON`) ولا يسحب كل صفوف `articles` إلى Node.
