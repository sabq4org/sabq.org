...
> آخر مراجعة: 2026-07-19 (إدارة ترخيص الكتّاب) | المالك: editorial
...
- **ترخيص كاتب الرأي على الحساب:** أعمدة `users.media_license_*`؛ المسارات `GET/POST /api/opinion-author/media-license` (رفع الملف عبر R2/S3 الخاص). البطاقة في `WriterMediaLicenseCard` على مساحة الكاتب؛ المهلة المعروضة `2026-07-31`. بعد الإرسال تظهر رسالة شكر وتُخفى الاستمارة.
...
- **سايدبار محرّر المقال:** على الشاشات الكبيرة `sticky` مع `self-start` و`max-h` مبنية على `100dvh` وهامش سفلي كافٍ (`pb-12`) حتى يظهر آخر قسم (SEO/الكلمات المفتاحية) بالكامل عند التمرير.
