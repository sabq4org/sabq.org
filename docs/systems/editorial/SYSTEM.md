...
- **منشئ المادة ≠ كاتب الرأي دائمًا:** لمقالات الرأي `authorId` = الكاتب الظاهر للقارئ، و`submitterId` = المستخدم الذي أنشأ المادة فعليًا (قد يكون الكاتب نفسه أو محررًا أدخلها نيابةً عنه). يثبّت الخادم `submitterId` عند الإنشاء ولا يغيّره عند التحرير أو الجدولة؛ وللسجلات القديمة الفارغة يُستعاد المنشئ من حدث `article_events.created` بدل نسبها إلى آخر محرر. بانر المحرر يعتمد على `enteredBy` من هذا المصدر فقط.
