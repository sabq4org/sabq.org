...
- **مراجع تحريرية ثابتة (PDF):** بطاقة `EditorialReferenceCard` على مساحة كاتب الرأي (`/dashboard/opinion-author`) وصفحة المراسل (`/dashboard/reporter/articles`). الملف يُخدم من الأصول العامة `client/public/editorial-refs/` (حالياً: نظام التعليم العام). ليست مكتبة CMS — لإضافة مرجع جديد انسخ PDF هناك وحدّث المكوّن أو مرّر `href`/`message`.
