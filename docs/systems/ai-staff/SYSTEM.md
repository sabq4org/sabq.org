...
> آخر مراجعة: 2026-09-25 | المالك: فريق التحرير التقني
...
- «كلّفه بمهمة» والمحوّلات لجداول الأنظمة (مثل radar_stories) في سجل الأعمال: مرحلة لاحقة — سجل الأعمال حاليًا من `ai_usage_logs` مباشرة. `journalist_tasks` أرشيف بعد إزالة الوكيل القديم.
- **مراسل (2026-09-25):** السجل الافتراضي يربط الموظف `murasil` بالبادئة الفعلية `ops-room-murasil` بدل `journalist-agent`. صفوف `ai_staff` المزروعة سابقًا تتقدم على الافتراضي؛ لا يحدّث هذا التغيير أي صف إنتاجي أو يدّعي تغيير بطاقة مزروعة تلقائيًا.
