...
## بوابة CI وSmoke بعد النشر

- Railway **Wait for CI** يجب أن يظل معطلاً حتى يُدمج فصل Smoke عن حدث `push`.
- فحوص ما قبل النشر هي workflows التي تعمل على `push` إلى `main`؛ عند تفعيل Wait for CI ينتظر Railway نجاحها، ويضع النشر `SKIPPED` إذا فشل أي منها.
- Smoke الإنتاجي لا يعمل على `push` ولا ينتظر مدة ثابتة. يبدأ من حدث GitHub `deployment_status` فقط عندما يرسل `railway-app[bot]` حالة `success` للبيئة الدقيقة `sabq.org / production`.
- الجدولة كل 6 ساعات والتشغيل اليدوي باقيان كمراقبة مستقلة.
- بعد دمج هذا العقد واختباره: فعّل `source.checkSuites=true` لخدمات الإنتاج المرتبطة بـ`main`، ثم اختبر commit فاشلاً (يُتخطى نشره) وcommit ناجحاً (ينشر ثم يعمل Smoke).
...
| `sabq-staging-web-next` | `Root Directory=/web-next` · `/web-next/railway.json` | API داخلي؛ `STAGING_NO_INDEX=true`؛ Basic Auth عبر `STAGING_ACCESS_TOKEN` |
...
read -rsp "STAGING_ACCESS_TOKEN: " STAGING_ACCESS_TOKEN; echo
curl -fsSI https://sabq-staging-web-next-staging.up.railway.app/ \
  -u "sabq:${STAGING_ACCESS_TOKEN}" \
...
بدون Basic Auth يجب أن يعيد الويب `401`، بينما يبقى `/health` عاماً لـ Railway.
