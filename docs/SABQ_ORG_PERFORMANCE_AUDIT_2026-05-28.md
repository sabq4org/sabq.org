...
## الموجة الثالثة — Wave 3 (2026-05-29، أداء الـ backend لبلوكات الذكاء في الصفحة الرئيسية)

بعد ملاحظة بطء متقطّع في ثلاثة مكوّنات أسفل الصفحة الرئيسية ("رحلتك المعرفية"، "مؤشرات الأسبوع"، وبلوك الحج)، تبيّن أن المكوّنات تُحمَّل بشكل كسول (lazy) ثم تجلب بياناتها من جهة العميل، فيظهر البطء عند أول جلب أو بعد انتهاء كاش قصير جداً على السيرفر. عُولج مصدران للبطء على مستوى الـ backend دون أي تغيير في الواجهة أو شكل/قيم النتائج.

### 1. `/api/ai-insights` — مؤشرات الأسبوع (بيانات عامة لكل الزوّار)

**المشكلة:**
- الاستعلامات الخمسة (الأكثر مشاهدة / تعليقاً / جدلاً / إيجابية / اختيار الذكاء) كانت تُنفَّذ **بالتتابع** — كل استعلام ينتظر سابقه.
- الكاش في الذاكرة كان **30 ثانية** فقط، فيدفع أول زائر بعد كل انتهاء صلاحية ثمن المسار الثقيل (5 استعلامات تجميعية على جداول كبيرة).

**التعديل في `server/routes.ts`:**
- تجميع الاستعلامات الخمسة المستقلّة في `Promise.all` لتعمل **بالتوازي** (لا يعتمد أيٌّ منها على نتيجة الآخر) → على مسار الكاش الفارغ تتحوّل ~5 رحلات تسلسلية إلى ~1.
- رفع TTL للكاش من **30 ثانية → 10 دقائق** (`memoryCache.set(cacheKey, result, 600000)`)؛ مبرَّر لأن مؤشرات 7 أيام مجمّعة تتغيّر ببطء شديد.

### 2. `/api/ai/insights/today` — رحلتك المعرفية (خاصة بكل مستخدم)

**المشكلة:**
- جلب بيانات المستخدم ثم القراءات ثم الإعجابات ثم التعليقات **بالتتابع** (request waterfall داخلي).
- لا يوجد كاش إطلاقاً، فكل تحميل للصفحة يضرب قاعدة البيانات بأربعة استعلامات لكل مستخدم.

**التعديل في `server/routes.ts`:**
- موازاة الاستعلامات الأربعة المستقلّة (المستخدم + القراءات + الإعجابات + التعليقات) عبر `Promise.all`. (استعلام "الاهتمامات" يبقى بعدها لأنه يعتمد على نتائج القراءات.)
- إضافة كاش **60 ثانية لكل مستخدم** (`insights:today:${userId}`) — قصير عمداً حتى لا تتجمّد أرقام المستخدم اليومية، لكنه يجعل البلوك (الموجود أسفل الصفحة) فورياً بعد أول تحميل.

### ملاحظة على نطاق الكاش

الكاش **ليس** موحّداً للواجهة — لكل نقطة مدّتها حسب طبيعتها:

| النقطة | الطبيعة | TTL |
|---|---|---:|
| `/api/ai-insights` | عام، يتغيّر ببطء | 10 دقائق |
| `/api/ai/insights/today` | خاص بكل مستخدم | 60 ثانية |
| `/api/hajj-block` | عام (كما هو سابقاً) | 5 دقائق |

محتوى الأخبار/الصفحة الرئيسية لم يُمسّ.

### الملفات المعدّلة في هذه الموجة

```
server/routes.ts  (نقطتا /api/ai-insights و /api/ai/insights/today)
```

### التحقق

- `npm run check` — **0 أخطاء** TypeScript.
- لا أخطاء lint.
- شكل الـ JSON المُعاد للواجهة وقيمه مطابقة تماماً للسابق؛ التغيير في *كيفية* الجلب لا في *ماذا* يُجلب.

### Commit

```
a743e4a  perf(insights): parallelize queries + raise cache TTLs on homepage AI blocks
```

دُفِع مباشرةً على `main` (بطلب صريح من المالك، استثناءً من سياسة PR في `AGENTS.md`).
