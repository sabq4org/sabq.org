...
// MARK: - Sabq Daily Brief

/// `GET /api/v1/daily-brief` — five concise, published stories. The endpoint
/// is public and adds at most two interest-based items when a member Bearer
/// token is present.
nonisolated struct APIDailyBrief: Decodable {
    let id: String
    let edition: String
    ...
    let updatedAt: String
    let estimatedReadingSeconds: Int
    let itemCount: Int
    let items: [Item]
    let personalization: Personalization

    nonisolated struct Item: Decodable, Identifiable {
        let id: String
        let position: Int
        let title: String
        let slug: String
        ...
        init(from decoder: Decoder) throws {
            let c = try decoder.container(keyedBy: FlexKey.self)
            id = (try? c.decode(String.self, forKey: FlexKey("id"))) ?? UUID().uuidString
            position = (try? c.decode(Int.self, forKey: FlexKey("position"))) ?? 0
            title = (try? c.decode(String.self, forKey: FlexKey("title"))) ?? ""
            slug = (try? c.decode(String.self, forKey: FlexKey("slug"))) ?? ""
            articleUrl = (try? c.decode(String.self, forKey: FlexKey("articleUrl")))
                ?? "https://sabq.org/article/\(slug)"
            summary = (try? c.decode(String.self, forKey: FlexKey("summary"))) ?? ""
            bullets = (try? c.decode([String].self, forKey: FlexKey("bullets"))) ?? []
            category = (try? c.decode(Category.self, forKey: FlexKey("category"))) ?? Category()
            imageUrl = try? c.decode(String.self, forKey: FlexKey("imageUrl"))
            imageFocalPoint = APIArticle.decodeFocalPoint(in: c)
            isAiGeneratedImage = (try? c.decode(Bool.self, forKey: FlexKey("isAiGeneratedImage"))) ?? false
            aiImageModel = try? c.decode(String.self, forKey: FlexKey("aiImageModel"))
            publishedAt = try? c.decode(String.self, forKey: FlexKey("publishedAt"))
            updatedAt = (try? c.decode(String.self, forKey: FlexKey("updatedAt"))) ?? ""
            isBreaking = (try? c.decode(Bool.self, forKey: FlexKey("isBreaking"))) ?? false
            isPersonalized = (try? c.decode(Bool.self, forKey: FlexKey("isPersonalized"))) ?? false
        }
    }

    nonisolated struct Category: Decodable {
        let id: String?
        let name: String
        let slug: String?
        let color: String?

        init(id: String? = nil, name: String = "عام", slug: String? = nil, color: String? = nil) {
            self.id = id
            self.name = name
            self.slug = slug
            self.color = color
        ...
        init(from decoder: Decoder) throws {
            let c = try decoder.container(keyedBy: FlexKey.self)
            id = try? c.decode(String.self, forKey: FlexKey("id"))
            name = (try? c.decode(String.self, forKey: FlexKey("name"))) ?? "عام"
            slug = try? c.decode(String.self, forKey: FlexKey("slug"))
            color = try? c.decode(String.self, forKey: FlexKey("color"))
        }
    }

    nonisolated struct Personalization: Decodable {
        let isPersonalized: Bool
        let personalizedItemCount: Int

        init(isPersonalized: Bool = false, personalizedItemCount: Int = 0) {
            self.isPersonalized = isPersonalized
            self.personalizedItemCount = personalizedItemCount
        }

        init(from decoder: Decoder) throws {
            let c = try decoder.container(keyedBy: FlexKey.self)
            isPersonalized = (try? c.decode(Bool.self, forKey: FlexKey("isPersonalized"))) ?? false
            personalizedItemCount = (try? c.decode(Int.self, forKey: FlexKey("personalizedItemCount"))) ?? 0
        }
    }
    ...
    init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: FlexKey.self)
        id = (try? c.decode(String.self, forKey: FlexKey("id"))) ?? ""
        edition = (try? c.decode(String.self, forKey: FlexKey("edition"))) ?? "morning"
        editionLabel = (try? c.decode(String.self, forKey: FlexKey("editionLabel"))) ?? "موجز اليوم"
        headline = (try? c.decode(String.self, forKey: FlexKey("headline"))) ?? "أهم الأخبار الآن"
        generatedAt = (try? c.decode(String.self, forKey: FlexKey("generatedAt"))) ?? ""
        updatedAt = (try? c.decode(String.self, forKey: FlexKey("updatedAt"))) ?? generatedAt
        estimatedReadingSeconds = (try? c.decode(Int.self, forKey: FlexKey("estimatedReadingSeconds"))) ?? 120
        items = (try? c.decode([Item].self, forKey: FlexKey("items"))) ?? []
        itemCount = (try? c.decode(Int.self, forKey: FlexKey("itemCount"))) ?? items.count
        personalization = (try? c.decode(Personalization.self, forKey: FlexKey("personalization")))
            ?? Personalization()
    }
}
