...
import { Activity, ArrowRight, CalendarDays, Cloud, Droplets, Flame, Gauge, LineChart, MapPin, Radio, Route as RouteIcon, Sparkles, Trophy, Tv, UserX, Users } from "lucide-react";
...
import { FollowControls } from "@/pages/SportsHub";
...
type AcCommentaryItem = { minute: number; extraMinute: number | null; goal: boolean; important: boolean; textAr: string; textEn: string; order: number };
type AcCommentary = { available: boolean; live: boolean; items: AcCommentaryItem[] };
type AcXg = { available: boolean; home: { xg: number; xgot: number }; away: { xg: number; xgot: number }; topPlayers: { name: string; location: "home" | "away"; xg: number }[] };
type AcWeather = { type: "actual" | "forecast"; temp: number | null; description: string; icon: string; humidity: string };
type AcAbsentee = { name: string; location: "home" | "away"; reason: string };
type AcMatchFacts = { available: boolean; statistics: AcStatistic[]; weather: AcWeather | null; absentees: AcAbsentee[]; halftime: { home: number; away: number } | null };
type AcForecast = { available: boolean; home: number; draw: number; away: number };
type AcPlayerFormMatch = { opponent: string; date: string | null; goals: number; assists: number; minutes: number; rating: number | null; xg: number | null };
type AcPlayerForm = { available: boolean; matches: AcPlayerFormMatch[] };
type AcPlayerMarket = { available: boolean; marketValue: number | null; currency: string; history: { time: number; value: number }[] };
type AcQualificationJourney = { team: AcTeam; available: boolean; title: string; subtitle: string; stats: { played: number; win: number; draw: number; lose: number; goalsFor: number; goalsAgainst: number }; timeline: { id: string; kind: string; date: string | null; title: string; subtitle: string | null; competition: string | null; round: string | null; opponent: AcTeam | null; isHome: boolean | null; goals: { for: number | null; against: number | null }; result: string | null; status: string | null }[] };
...
function Hero({ title, subtitle, image }: { title: string; subtitle?: string | null; image?: string | null }) {
  return <section className="relative overflow-hidden bg-gradient-to-bl from-emerald-600 via-emerald-500 to-teal-600 text-white"><div className="absolute -top-24 -left-16 h-72 w-72 rounded-full bg-amber-300/20 blur-3xl" /><div className="relative mx-auto max-w-6xl px-4 py-8"><Link href="/asian-cup" className="mb-5 inline-flex items-center gap-1 text-xs text-emerald-50/80 hover:text-white"><ArrowRight className="h-4 w-4" />العودة إلى كأس آسيا</Link><div className="flex items-center gap-5">{image ? <span className="h-24 w-24 shrink-0 rounded-full bg-white p-2 shadow-xl"><img src={image} alt="" className="h-full w-full object-contain" /></span> : null}<div><h1 className="text-2xl font-black sm:text-4xl">{title}</h1>{subtitle ? <p className="mt-2 text-sm text-emerald-50/85">{subtitle}</p> : null}</div></div></div></section>;
}
...
/** التعليق المباشر — أحدثه أعلى؛ الأهداف/اللحظات المهمّة مُبرَزة. */
function CommentaryTab({ fixtureId, live }: { fixtureId: number; live: boolean }) {
  const { data, isLoading } = useQuery<AcCommentary>({ queryKey: [`/api/asian-cup/commentary/${fixtureId}`], refetchInterval: live ? 15_000 : false });
  if (isLoading) return <div className="space-y-2">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-12 w-full" />)}</div>;
  const items = Array.isArray(data?.items) ? [...data.items].sort((a, b) => b.order - a.order) : [];
  if (items.length === 0) return <Empty text={live ? "التعليق اللحظي يبدأ مع صافرة البداية." : "لا يتوفر تعليق لهذه المباراة."} />;
  return (
    <div className="space-y-2">
      {items.map((item, i) => {
        const highlight = item.goal || item.important;
        return (
          <div key={i} className={`flex items-start gap-3 rounded-xl px-3 py-2.5 ${highlight ? "bg-emerald-500/10 ring-1 ring-emerald-500/20" : "bg-muted/40"}`}>
            <Badge variant="secondary" className="min-w-[3rem] shrink-0 justify-center tabular-nums" dir="ltr">{item.minute > 0 ? `${item.minute}'` : "—"}{item.minute > 0 && item.extraMinute ? `+${item.extraMinute}` : ""}</Badge>
            <p className="min-w-0 flex-1 text-sm leading-relaxed">{item.textAr || item.textEn}</p>
          </div>
        );
      })}
    </div>
...
/** شريط إحصاء ثنائي — أخضر للمضيف يمينًا، سماوي للضيف يسارًا. */
function StatBar({ stat }: { stat: AcStatistic }) {
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between text-sm"><span className="w-12 font-black tabular-nums">{stat.home}</span><span className="text-xs text-muted-foreground">{stat.label}</span><span className="w-12 text-left font-black tabular-nums">{stat.away}</span></div>
      <div className="relative h-1.5 rounded-full bg-muted"><div className="absolute bottom-0 top-0 end-1/2 rounded-s-full bg-emerald-500" style={{ width: `${(home / max) * 50}%` }} /><div className="absolute bottom-0 top-0 start-1/2 rounded-e-full bg-sky-500" style={{ width: `${(away / max) * 50}%` }} /></div>
    </div>
  );
}

/** الأهداف المتوقعة xG (SportMonks) — تُخفى حين available:false. */
function XgCard({ fixtureId, fixture }: { fixtureId: number; fixture: AcFixture }) {
  const { data } = useQuery<AcXg>({ queryKey: [`/api/asian-cup/xg/${fixtureId}`], refetchInterval: fixture.status.live ? 30_000 : false });
  if (!data?.available) return null;
  const teamLogo = (loc: "home" | "away") => (loc === "home" ? fixture.home.logo : fixture.away.logo);
  return (
    <div className="space-y-2.5 rounded-xl bg-gradient-to-l from-emerald-500/10 to-sky-500/10 px-3.5 py-3 ring-1 ring-border/60">
      <StatBar stat={{ key: "xg", label: "الأهداف المتوقعة (xG)", home: data.home.xg.toFixed(2), away: data.away.xg.toFixed(2) }} />
      {(data.home.xgot > 0 || data.away.xgot > 0) && <div className="flex items-center justify-between text-[11px] text-muted-foreground"><span className="w-12 tabular-nums">{data.home.xgot.toFixed(2)}</span><span>على المرمى (xGoT)</span><span className="w-12 text-left tabular-nums">{data.away.xgot.toFixed(2)}</span></div>}
      {data.topPlayers.length > 0 && <div className="space-y-1 border-t border-border/60 pt-2"><p className="text-[10px] text-muted-foreground">الأعلى خطورة (xG)</p>{data.topPlayers.slice(0, 3).map((p, i) => <div key={i} className="flex items-center justify-between text-[11px]"><span className="flex min-w-0 items-center gap-1.5"><img src={teamLogo(p.location)} alt="" className="h-3.5 w-3.5 shrink-0 object-contain" loading="lazy" /><span className="truncate">{p.name}</span></span><span className="shrink-0 font-bold tabular-nums">{p.xg.toFixed(2)}</span></div>)}</div>}
    </div>
  );
}

function WeatherCard({ weather }: { weather: AcWeather }) {
  return (
    <div className="flex items-center gap-3 rounded-xl bg-muted/40 px-3.5 py-2.5">
      {weather.icon ? <img src={weather.icon} alt="" className="h-9 w-9 shrink-0 object-contain" loading="lazy" /> : <Cloud className="h-7 w-7 shrink-0 text-sky-500" />}
      <div className="min-w-0 flex-1"><p className="text-sm font-bold">{weather.description}{weather.type === "forecast" && <span className="text-[10px] font-normal text-muted-foreground"> · توقّع</span>}</p><div className="mt-0.5 flex items-center gap-3 text-[11px] text-muted-foreground">{weather.temp != null && <span className="tabular-nums">{weather.temp}°م</span>}{weather.humidity && <span className="flex items-center gap-0.5"><Droplets className="h-3 w-3" /> {weather.humidity}</span>}</div></div>
    </div>
  );
}

function AbsenteesSection({ absentees, fixture }: { absentees: AcAbsentee[]; fixture: AcFixture }) {
  const col = (team: AcTeam, location: "home" | "away") => {
    const list = absentees.filter((a) => a.location === location);
    return (
      <div className="min-w-0">
        <div className="mb-1.5 flex items-center gap-1.5"><img src={team.logo} alt="" className="h-4 w-4 shrink-0 object-contain" loading="lazy" /><span className="truncate text-xs font-bold">{team.name}</span></div>
        <ul className="space-y-1">{list.length === 0 ? <li className="text-[11px] text-muted-foreground">—</li> : list.map((a, i) => <li key={i} className="truncate text-[11px] text-muted-foreground"><span className="font-semibold text-foreground">{a.name}</span>{a.reason ? ` — ${a.reason}` : ""}</li>)}</ul>
      </div>
    );
  };
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-1.5"><UserX className="h-3.5 w-3.5 text-rose-500" /><h4 className="text-xs font-bold text-rose-600 dark:text-rose-300">الغيابات</h4></div>
      <div className="grid grid-cols-2 gap-3">{col(fixture.home, "home")}{col(fixture.away, "away")}</div>
    </div>
  );
}

/** الإحصائيات المُثراة: xG + طقس + غيابات + إحصاء المباراة (SportMonks أعمق ثم API-Football). */
function StatsTab({ detail }: { detail: AcMatchDetail }) {
  const f = detail.fixture;
  const { data: facts } = useQuery<AcMatchFacts>({ queryKey: [`/api/asian-cup/match-facts/${f.id}`], refetchInterval: f.status.live ? 30_000 : false });
  const stats = f.status.live && detail.statistics.length > 0 ? detail.statistics : (facts?.statistics?.length ?? 0) > 0 ? facts!.statistics : detail.statistics;
  const weather = facts?.weather ?? null;
  const hasAbsentees = (facts?.absentees?.length ?? 0) > 0;
  const nothing = stats.length === 0 && !weather && !hasAbsentees;
  return (
    <Card className="space-y-4 p-5">
      <XgCard fixtureId={f.id} fixture={f} />
      {weather && <WeatherCard weather={weather} />}
      {hasAbsentees && facts && <AbsenteesSection absentees={facts.absentees} fixture={f} />}
      {stats.length > 0 && <div className="space-y-3">{(weather || hasAbsentees) && <div className="border-t border-border/60 pt-1" />}{stats.map((s) => <StatBar key={s.key} stat={s} />)}</div>}
      {nothing && <Empty text="تظهر إحصاءات المباراة مع انطلاقها." />}
    </Card>
  );
}

/** التوقعات: احتمالات النموذج (API-Football) أو forecast الخادم عند غيابها. */
function PredictionTab({ detail }: { detail: AcMatchDetail }) {
  const f = detail.fixture;
  const { data: forecast } = useQuery<AcForecast>({ queryKey: [`/api/asian-cup/forecast/${f.id}`], enabled: !detail.prediction });
  const ft = detail.prediction ?? (forecast?.available ? { home: forecast.home, draw: forecast.draw, away: forecast.away } : null);
  if (!ft) return <Empty text="يظهر توقع النموذج قبل انطلاق المباراة." />;
  return (
    <Card className="p-5">
      <h2 className="mb-4 font-black">احتمالات المباراة</h2>
      <StatGrid items={[[f.home.name, `${ft.home}%`], ["التعادل", `${ft.draw}%`], [f.away.name, `${ft.away}%`]]} />
      <p className="mt-3 text-[11px] text-muted-foreground">تقدير خوارزمي للاسترشاد فقط — وليس ترجيحًا تحريريًا.</p>
    </Card>
  );
}
...
export function AsianCupMatchPage() {
  ...
  <Tabs defaultValue={f.status.live ? "commentary" : f.status.finished ? "events" : "prediction"} dir="rtl">
    <TabsList className="mb-4 h-auto w-full flex-wrap justify-start gap-1">
      {(f.status.live || f.status.finished) && <TabsTrigger value="commentary" className="gap-1">{f.status.live && <Radio className="h-3 w-3 animate-pulse text-red-500" />}{f.status.live ? "مباشر" : "التعليق"}</TabsTrigger>}
    ...
    {(f.status.live || f.status.finished) && <TabsContent value="commentary" className="mt-0"><CommentaryTab fixtureId={f.id} live={f.status.live} /></TabsContent>}
    ...
    <TabsContent value="stats" className="mt-0"><StatsTab detail={data} /></TabsContent>
    ...
    <TabsContent value="prediction" className="mt-0"><PredictionTab detail={data} /></TabsContent>
...
/** رحلة تأهّل المنتخب — تُخفى كليًّا حين available:false. */
function QualificationJourney({ teamId }: { teamId: number }) {
  const { data } = useQuery<AcQualificationJourney>({ queryKey: [`/api/asian-cup/team/${teamId}/qualification`] });
  if (!data?.available) return null;
  const s = data.stats;
  return (
    <Card className="space-y-4 p-5">
      <div className="flex items-center gap-2"><RouteIcon className="h-5 w-5 text-emerald-600" /><div><h2 className="font-black">{data.title || "رحلة التأهل"}</h2>{data.subtitle && <p className="text-xs text-muted-foreground">{data.subtitle}</p>}</div></div>
      <StatGrid items={[["مباريات", s.played], ["فوز", s.win], ["تعادل", s.draw], ["خسارة", s.lose], ["له", s.goalsFor], ["عليه", s.goalsAgainst]]} />
      {data.timeline.length > 0 && (
        <ol className="space-y-2 border-t border-border/60 pt-3">
          {data.timeline.slice(0, 12).map((t) => (
            <li key={t.id} className="flex items-center gap-3 rounded-lg bg-muted/35 p-2.5 text-sm">
              {t.opponent?.logo ? <img src={t.opponent.logo} alt="" className="h-7 w-7 shrink-0 object-contain" loading="lazy" /> : <span className="h-7 w-7 shrink-0 rounded-full bg-muted" />}
              <div className="min-w-0 flex-1"><p className="truncate font-bold">{t.title}</p>{(t.subtitle || t.round || t.competition) && <p className="truncate text-[11px] text-muted-foreground">{[t.round, t.competition, t.subtitle].filter(Boolean).join(" · ")}</p>}</div>
              {t.goals.for != null && t.goals.against != null && <span className="shrink-0 font-black tabular-nums" dir="ltr">{t.goals.for} - {t.goals.against}</span>}
              {t.result && <Badge variant="secondary" className="shrink-0">{t.result}</Badge>}
            </li>
          ))}
        </ol>
      )}
...
function fmtMarket(value: number, currency: string): string {
  const v = value >= 1_000_000 ? `${(value / 1_000_000).toFixed(1)}M` : `${Math.round(value / 1_000)}K`;
  return `${v} ${currency}`;
}

/** آخر مبارياته (SportMonks) — تُخفى حين available:false أو لا مباريات. */
function PlayerFormSection({ playerId }: { playerId: number }) {
  const { data } = useQuery<AcPlayerForm>({ queryKey: [`/api/asian-cup/player/${playerId}/form`] });
  const matches = Array.isArray(data?.matches) ? data.matches : [];
  if (!data?.available || matches.length === 0) return null;
  return (
    <Card className="p-5">
      <h2 className="mb-4 flex items-center gap-2 font-black"><LineChart className="h-5 w-5 text-emerald-600" />آخر المباريات</h2>
      <div className="space-y-1.5">
        {matches.slice(0, 8).map((m, i) => (
          <div key={i} className="flex items-center gap-3 rounded-lg bg-muted/35 p-2.5 text-sm">
            <span className="min-w-0 flex-1 truncate font-bold">{m.opponent || "—"}</span>
            {m.date && <span className="shrink-0 text-[11px] text-muted-foreground" dir="ltr">{m.date.slice(0, 10)}</span>}
            <span className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">{m.goals > 0 && <span>{m.goals} ⚽</span>}{m.rating != null && <Badge>{m.rating.toFixed(1)}</Badge>}{m.xg != null && <span dir="ltr">xG {m.xg.toFixed(2)}</span>}</span>
          </div>
        ))}
      </div>
    </Card>
...
/** القيمة السوقية + منحنى تطوّرها (TheSports) — تُخفى حين available:false. */
function PlayerMarketSection({ playerId }: { playerId: number }) {
  const { data } = useQuery<AcPlayerMarket>({ queryKey: [`/api/asian-cup/player/${playerId}/market`] });
  if (!data?.available || !data.marketValue) return null;
  const history = Array.isArray(data.history) ? data.history : [];
  const max = Math.max(1, ...history.map((h) => h.value));
  return (
    <Card className="p-5">
      <h2 className="mb-4 font-black">القيمة السوقية</h2>
      <div className="flex items-baseline gap-2"><span className="text-3xl font-black tabular-nums text-emerald-700">{fmtMarket(data.marketValue, data.currency)}</span></div>
      {history.length > 1 && (
        <div dir="ltr" className="mt-4 flex h-24 items-end gap-1">
          {history.slice(-24).map((h, i) => <div key={i} className="flex-1 rounded-t bg-emerald-500/70" style={{ height: `${(h.value / max) * 100}%` }} title={fmtMarket(h.value, data.currency)} />)}
        </div>
      )}
    </Card>
  );
...
export function AsianCupTeamPage() {
  ...
  return <Shell><Hero title={data.team.name} subtitle={[data.fifaRank && `تصنيف فيفا #${data.fifaRank.rank}`, data.stats.groupName, data.coach && `المدرب: ${data.coach}`].filter(Boolean).join(" · ")} image={data.team.logo} /><div className="mx-auto max-w-6xl space-y-6 px-4 py-8"><div className="flex items-center justify-end"><FollowControls kind="team" refId={data.team.id} refName={data.team.name} refLogo={data.team.logo} size="md" /></div><Card className="p-5"><StatGrid items={[["المركز", data.stats.rank ?? "–"], ["النقاط", data.stats.points], ["فوز", data.stats.win], ["تعادل", data.stats.draw], ["خسارة", data.stats.lose], ["له", data.stats.goalsFor], ["عليه", data.stats.goalsAgainst], ["الفارق", data.stats.goalsDiff]]} /></Card>
  ...
  <QualificationJourney teamId={teamId} />
...
export function AsianCupPlayerPage() {
  ...
  <PlayerFormSection playerId={playerId} />
  <PlayerMarketSection playerId={playerId} />
