...
import { Activity, ArrowRight, CalendarDays, MapPin, Trophy, Users } from "lucide-react";
...
type AcMatchDetail = { fixture: AcFixture; events: AcEvent[]; statistics: AcStatistic[]; ratings: AcRating[]; lineups: AcLineup[]; headToHead: AcFixture[]; manOfTheMatch: AcRating | null; prediction: { home: number; draw: number; away: number } | null };
...
export function AsianCupTeamPage() {
  ...
  return <Shell><Hero title={data.team.name} subtitle={[data.stats.groupName, data.coach && `المدرب: ${data.coach}`].filter(Boolean).join(" · ")} image={data.team.logo} /><div className="mx-auto max-w-6xl space-y-6 px-4 py-8"><Card className="p-5"><StatGrid items={[["المركز", data.stats.rank ?? "–"], ["النقاط", data.stats.points], ["فوز", data.stats.win], ["تعادل", data.stats.draw], ["خسارة", data.stats.lose], ["له", data.stats.goalsFor], ["عليه", data.stats.goalsAgainst], ["الفارق", data.stats.goalsDiff]]} /></Card>
