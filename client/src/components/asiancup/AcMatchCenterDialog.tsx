...
import { LiveMinute } from "@/components/worldcup/LiveMinute";
import { PenaltyResult } from "@/components/worldcup/PenaltyResult";

export interface AcMatchCenterDialogProps {
...
function EventsTab({
  ...
  const { data: facts } = useQuery<WcMatchFacts>({
    queryKey: [`/api/asian-cup/match-facts/${detail.fixture.id}`],
...
function LineupsTab({
  ...
  const { data: expected } = useQuery<WcExpectedLineups>({
    queryKey: [`/api/asian-cup/match/${fixture.id}/expected-lineup`],
...
function XgCard({ fixtureId, fixture }: { fixtureId: number; fixture: WcFixture }) {
  const { data } = useQuery<WcXg>({
    queryKey: [`/api/asian-cup/xg/${fixtureId}`],
...
function StatsTab({ detail }: { detail: WcMatchDetail }) {
  ...
  const { data: tsStats } = useQuery<{ available: boolean; team: WcStatistic[] }>({
    queryKey: [`/api/asian-cup/match/${detail.fixture.id}/stats`],
...
function ForecastBlocks({ detail }: { detail: WcMatchDetail }) {
  const { data, isLoading } = useQuery<WcForecast>({
    queryKey: [`/api/asian-cup/forecast/${detail.fixture.id}`],
    refetchInterval: detail.fixture.status.live ? 30_000 : false,
  });
  if (isLoading) {
    return (
...
function TsRatingsFallback({ fixtureId, live }: { fixtureId: number; live: boolean }) {
  const { data, isLoading } = useQuery<WcMatchPlayerStats>({
    queryKey: [`/api/asian-cup/match/${fixtureId}/player-stats`],
...
function MomentumTab({
  ...
  const { data, isLoading } = useQuery<WcMomentum>({
    queryKey: [`/api/asian-cup/momentum/${fixtureId}`],
...
function PressureTab({
  ...
  const { data, isLoading } = useQuery<WcPressure>({
    queryKey: [`/api/asian-cup/pressure/${fixtureId}`],
    refetchInterval: live ? 12_000 : false,
  });
...
function CommentaryTab({ fixtureId, live }: { fixtureId: number; live: boolean }) {
  const { data, isLoading } = useQuery<WcCommentary>({
    queryKey: [`/api/asian-cup/commentary/${fixtureId}`],
...
function MatchTvSection({ fixtureId }: { fixtureId: number }) {
  const { data } = useQuery<{ available: boolean; channels: WcTvChannel[] }>({
    queryKey: [`/api/asian-cup/match/${fixtureId}/tv`],
  });
...
function MatchRefereeSection({ fixtureId }: { fixtureId: number }) {
  const { data } = useQuery<WcMatchReferee>({
    queryKey: [`/api/asian-cup/match/${fixtureId}/referee`],
    staleTime: 5 * 60 * 1000,
  });
  if (!data?.available) return null;
  const s = data.stats;
...
export function AcMatchCenterDialog({
  fixtureId,
  onClose,
  onOpenPlayer = () => {},
}: AcMatchCenterDialogProps) {
  const { data: detail, isLoading } = useQuery<WcMatchDetail>({
    queryKey: [`/api/asian-cup/match/${fixtureId}`],
