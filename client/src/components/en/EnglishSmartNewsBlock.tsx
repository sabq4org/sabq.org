                  ...
                  <div className="relative h-48 overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3 flex gap-2 flex-wrap z-10">
                      {article.newsType === "breaking" ? (
                        ...
                        <Badge 
                          className="gap-1 bg-amber-600 hover:bg-amber-700 text-white border-0 font-medium" 
                      ...
                      {isNewArticle(article.publishedAt) && (
                        <Badge 
                          className="gap-1 bg-emerald-500 hover:bg-emerald-600 text-white border-emerald-600 animate-pulse" 
...
function FeaturedLayout({ articles, blockId }: { articles: ArticleResult[]; blockId: string }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-1" data-testid={`smart-block-featured-${blockId}`}>
      <Link href={`/en/article/${featured.englishSlug || featured.slug}`} className="lg:col-span-3">
        <Card 
      ...
      {sideArticles.length > 0 && (
        <div className="lg:col-span-2 grid grid-cols-2 gap-1">
          {sideArticles.map((article) => (
            <Link key={article.id} href={`/en/article/${article.englishSlug || article.slug}`} className="col-span-1">
              <Card 
