...
const TEXT = {
  ar: {
    ...
    humanLabel: "بشري",
    bodyHelp: "كم يبلغ دور الذكاء الاصطناعي في صياغة نص المقال؟ النسبة المتبقية تمثل المراجعة والكتابة البشرية.",
    coverHelp: "هل الصورة الرئيسية مُولَّدة بالذكاء الاصطناعي أم من مصدر بشري؟",
    seoHelp: "هل تم توليد بيانات SEO (العنوان، الوصف، الكلمات المفتاحية) آلياً أم يدوياً؟",
    fullyHuman: "بشري بالكامل",
    fullyAi: "ذكاء اصطناعي بالكامل",
  ...
  en: {
    ...
    aiLabel: "AI",
    humanLabel: "Human",
    bodyHelp: "How much did AI contribute to drafting this article's body? The remainder is human writing and review.",
    coverHelp: "Was the cover image AI-generated or sourced from a human?",
    seoHelp: "Were the SEO meta (title, description, keywords) generated by AI or hand-written?",
    fullyHuman: "Fully human",
    fullyAi: "Fully AI",
  ...
  ur: {
    ...
    aiLabel: "AI",
    humanLabel: "انسانی",
    bodyHelp: "مضمون کے متن میں AI کا کتنا حصہ ہے؟ باقی فیصد انسانی تحریر اور جائزہ کی نمائندگی کرتا ہے۔",
    coverHelp: "کیا سرورق تصویر AI سے بنی ہے یا انسانی ماخذ سے؟",
    seoHelp: "کیا SEO ڈیٹا AI سے تیار ہوا یا دستی طور پر؟",
    fullyHuman: "مکمل انسانی",
    fullyAi: "مکمل AI",
...
// Visualizes "AI X% · Human Y%" as a single horizontal split bar.
// The amber-filled portion = AI contribution, the rest stays emerald
// background so it's instantly readable as "rest is human".
function SplitBar({
  aiPct,
  aiLabel,
  humanLabel,
  ...
  const clamped = Math.max(0, Math.min(100, aiPct));
  const humanPct = 100 - clamped;
  return (
    <div className="space-y-1" data-testid={testId}>
      ...
      <div className="h-2 rounded-full overflow-hidden flex bg-emerald-200/70 dark:bg-emerald-900/40 border border-emerald-300/50 dark:border-emerald-800/40">
        {clamped > 0 && (
          <div
      ...
      <div className="container mx-auto max-w-6xl px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* AI Footprint */}
            <Card data-testid="block-ai-footprint" className="border-blue-200/60 dark:border-blue-900/40">
              <CardHeader className="bg-gradient-to-l from-blue-50/60 to-transparent dark:from-blue-950/20 dark:to-transparent rounded-t-xl border-b border-blue-100/60 dark:border-blue-900/40">
              ...
              <CardContent className="space-y-5 pt-5">
                {/* Total — big split bar showing AI vs human contribution */}
                <div className="rounded-lg border bg-gradient-to-bl from-blue-50/40 via-card to-emerald-50/30 dark:from-blue-950/20 dark:via-card dark:to-emerald-950/15 p-4 space-y-3">
                  <div className="flex items-center justify-between gap-2">
                    <div className="text-sm font-medium">{t.aiTotal}</div>
                    <div className="text-2xl font-bold tabular-nums text-amber-600 dark:text-amber-400" data-testid="text-ai-total-pct">
                      {ai.percentages.total}%
                    </div>
                  </div>
                  <SplitBar aiPct={ai.percentages.total} aiLabel={t.aiLabel} humanLabel={t.humanLabel} testId="bar-ai-total" />
                  <div className="text-xs text-muted-foreground leading-relaxed" data-testid="text-ai-explanation">
                    {ai.explanation[language]}
                  </div>
                </div>

                {/* 3 detail rows — each row tinted in its accent colour + own split bar */}
                <div className="space-y-3">

                  {/* Body */}
                  <div
                    className="rounded-lg border border-blue-200/70 dark:border-blue-900/50 bg-blue-50/30 dark:bg-blue-950/10 p-3.5 space-y-2.5"
                    data-testid="row-ai-body"
                  >
                    <div className="flex items-start gap-3">
                      <div className="w-10 h-10 rounded-md bg-blue-500/15 text-blue-600 dark:text-blue-400 flex items-center justify-center flex-shrink-0">
                        <Sparkles className="h-5 w-5" />
                      </div>
                      <div className="flex-1 min-w-0 space-y-0.5">
                        <div className="text-sm font-semibold">{t.body}</div>
                        <div className="text-xs text-muted-foreground" data-testid="text-ai-body-tier">
                          {t.aiBody[ai.body.tier]}
                        </div>
                        {ai.body.aiEditCount > 0 && (
                          <div className="text-[11px] text-muted-foreground inline-flex items-center gap-1" data-testid="text-ai-edit-count">
                            <PencilLine className="h-3 w-3" />
                            {t.aiEditsCount(ai.body.aiEditCount)}
                          </div>
                        )}
                      </div>
                      <span className="text-sm font-semibold tabular-nums text-amber-600 dark:text-amber-400" data-testid="text-ai-body-pct">
                        {ai.percentages.body}%
                      </span>
                    </div>
                    <SplitBar aiPct={ai.percentages.body} aiLabel={t.aiLabel} humanLabel={t.humanLabel} testId="bar-ai-body" />
                    <p className="text-[11px] text-muted-foreground leading-relaxed">{t.bodyHelp}</p>
                  </div>

                  {/* Cover */}
                  <div
                    className="rounded-lg border border-violet-200/70 dark:border-violet-900/50 bg-violet-50/30 dark:bg-violet-950/10 p-3.5 space-y-2.5"
                    data-testid="row-ai-cover"
                  >
                    <div className="flex items-start gap-3">
                      <div className="w-10 h-10 rounded-md bg-violet-500/15 text-violet-600 dark:text-violet-400 flex items-center justify-center flex-shrink-0">
                      ...
                      <span className="text-sm font-semibold tabular-nums text-amber-600 dark:text-amber-400" data-testid="text-ai-cover-pct">
                        {ai.percentages.cover}%
                      </span>
                    </div>
                    <SplitBar aiPct={ai.percentages.cover} aiLabel={t.aiLabel} humanLabel={t.humanLabel} testId="bar-ai-cover" />
                    <p className="text-[11px] text-muted-foreground leading-relaxed">{t.coverHelp}</p>
                  </div>

                  {/* SEO */}
                  <div
                    className="rounded-lg border border-emerald-200/70 dark:border-emerald-900/50 bg-emerald-50/30 dark:bg-emerald-950/10 p-3.5 space-y-2.5"
                    data-testid="row-ai-seo"
                  >
                    <div className="flex items-start gap-3">
                      <div className="w-10 h-10 rounded-md bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 flex items-center justify-center flex-shrink-0">
                        <SearchIcon className="h-5 w-5" />
                      </div>
                      <div className="flex-1 min-w-0 space-y-0.5">
                        <div className="text-sm font-semibold">{t.seo}</div>
                        <div className="text-xs text-muted-foreground" data-testid="text-ai-seo">
                          {ai.seo.status || (ai.percentages.seo === 0 ? t.aiSeoManual : "—")}
                          {ai.seo.provider && <span> · {ai.seo.provider}{ai.seo.model ? ` · ${ai.seo.model}` : ""}</span>}
                        </div>
                      </div>
                      <span className="text-sm font-semibold tabular-nums text-amber-600 dark:text-amber-400" data-testid="text-ai-seo-pct">
                        {ai.percentages.seo}%
                      </span>
                    </div>
                    <SplitBar aiPct={ai.percentages.seo} aiLabel={t.aiLabel} humanLabel={t.humanLabel} testId="bar-ai-seo" />
                    <p className="text-[11px] text-muted-foreground leading-relaxed">{t.seoHelp}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-slate-200/70 dark:border-slate-800/60">
              <CardHeader className="bg-gradient-to-l from-slate-50/80 to-transparent dark:from-slate-900/30 dark:to-transparent rounded-t-xl border-b border-slate-100/70 dark:border-slate-800/50">
                <CardTitle className="flex items-center gap-2 text-lg">
            ...
            {passport.aiImageGenerations.length > 0 && (
              <Card className="border-violet-200/60 dark:border-violet-900/50">
                <CardHeader className="bg-gradient-to-l from-violet-50/70 to-transparent dark:from-violet-950/20 dark:to-transparent rounded-t-xl border-b border-violet-100/60 dark:border-violet-900/40">
                  <CardTitle className="flex items-center gap-2 text-lg">
          ...
          <div className="space-y-6 lg:sticky lg:top-20 lg:self-start lg:h-fit">
            ...
            {/* Trust badge prominent — tinted by tier */}
            <Card
              className={
                passport.trustBadge.tier === "human_edited"
                  ? "border-green-200/70 dark:border-green-900/50"
                  : passport.trustBadge.tier === "ai_assisted"
            ...
            >
              <CardHeader
                className={
                  passport.trustBadge.tier === "human_edited"
                    ? "bg-gradient-to-l from-green-50/70 to-transparent dark:from-green-950/25 dark:to-transparent rounded-t-xl border-b border-green-100/60 dark:border-green-900/40"
                    : passport.trustBadge.tier === "ai_assisted"
                      ? "bg-gradient-to-l from-blue-50/70 to-transparent dark:from-blue-950/25 dark:to-transparent rounded-t-xl border-b border-blue-100/60 dark:border-blue-900/40"
                      : "bg-gradient-to-l from-amber-50/70 to-transparent dark:from-amber-950/25 dark:to-transparent rounded-t-xl border-b border-amber-100/60 dark:border-amber-900/40"
                }
              >
                <CardTitle className="flex items-center gap-2 text-lg">
                  <TrustBadgeIcon tier={passport.trustBadge.tier} />
              ...
              <CardContent className="space-y-3 pt-5">
                ...
                {passport.trustBadge.credibilityScore !== null && (
                  <div
                    className="rounded-md border bg-gradient-to-bl from-amber-50/50 via-card to-emerald-50/40 dark:from-amber-950/15 dark:via-card dark:to-emerald-950/15 p-3 space-y-1"
                    data-testid="block-credibility"
                  >
            ...
            {showSources && (
              <Card className="border-sky-200/60 dark:border-sky-900/50">
                <CardHeader className="bg-gradient-to-l from-sky-50/70 to-transparent dark:from-sky-950/20 dark:to-transparent rounded-t-xl border-b border-sky-100/60 dark:border-sky-900/40">
                  <CardTitle className="flex items-center gap-2 text-lg">
            ...
            <Card className="border-emerald-200/60 dark:border-emerald-900/50">
              <CardHeader className="bg-gradient-to-l from-emerald-50/70 to-transparent dark:from-emerald-950/20 dark:to-transparent rounded-t-xl border-b border-emerald-100/60 dark:border-emerald-900/40">
                <CardTitle className="flex items-center gap-2 text-lg">
                  <SearchIcon className="h-5 w-5 text-emerald-600 dark:text-emerald-400" />
                  {t.sectionSeo}
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-5">
        ...
        {/* FULL-WIDTH TIMELINE */}
        <Card className="border-slate-200/70 dark:border-slate-800/60">
          <CardHeader className="bg-gradient-to-l from-slate-100/80 via-slate-50/50 to-transparent dark:from-slate-900/40 dark:via-slate-900/20 dark:to-transparent rounded-t-xl border-b border-slate-100/70 dark:border-slate-800/50">
            <CardTitle className="flex items-center gap-2 text-lg">
