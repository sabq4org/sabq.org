/**
 * قائمة «توقّعاتي» لدوري روشن — نسخة قائمة المونديال (MyPredictionsList):
 * بطاقة لكل توقّع بشريط حالة علوي (+نقاط من الجائزة المقسومة / لم تُصب /
 * قيد الانتظار) وكتلتي نتيجة (توقّعي × النتيجة).
 */
import { Clock, Trophy, X } from "lucide-react";
import { Card } from "@/components/ui/card";
import { formatKickoffDay } from "../rslTypes";
import { formatNumber } from "@/lib/format";
import type { MyPredictionRow } from "./rslPredictionsTypes";
...
export function RslMyPredictions({ predictions, isLoading }: Props) {
  if (isLoading) {
    return (
      ...
      {predictions.map((p) => {
        ...
        return (
          <Card
            ...
            data-testid={`rsl-my-pred-${p.fixtureId}`}
          >
            ...
            >
              <span className="text-[11px] font-semibold text-muted-foreground">
                {p.kickoffAt ? formatKickoffDay(p.kickoffAt) : "دوري روشن السعودي"}
              </span>
              {settled ? (
                isWin ? (
