/**
 * لوحة متصدّري التوقّعات — نفس لوحة المونديال (PredictionsLeaderboard):
 * تيجان للمراكز الثلاثة الأولى، صف المستخدم الحالي مميّز، وعدّادات
 * الإصابات الدقيقة والمباريات.
 */
import { Crown, Trophy } from "lucide-react";
import { formatNumber } from "@/lib/format";
import type { KcLeaderRow } from "./kcPredictionsTypes";
...
export function KcPredictionsLeaderboard({ leaders, currentUserId, isLoading }: Props) {
  if (isLoading) {
    return (
      ...
      {leaders.map((row) => {
        ...
        return (
          <div
            ...
            data-testid={`kc-leader-${row.rank}`}
          >
            <div
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[11px] text-muted-foreground">
                {formatNumber(row.exact)} نتيجة دقيقة · {formatNumber(row.predictions)} مباراة
              </p>
            </div>
