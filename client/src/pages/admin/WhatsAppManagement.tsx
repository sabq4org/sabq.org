...
export default function WhatsAppManagement() {
  ...
  const { data: stats, isLoading: statsLoading } = useQuery<StatsData>({
    ...
    retry: 1,
    staleTime: 30000,
    enabled: !!user && (user.role === 'admin' || user.role === 'system_admin' || user.role === 'manager'),
  });
  ...
  } = useQuery<{ logs: WhatsappWebhookLog[]; total: number }>({
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground mt-1">
                إدارة رموز واتساب ومراقبة الرسائل الواردة
              </p>
            </div>
          </div>
          ...
          <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
            <AlertDialogContent dir="rtl" data-testid="dialog-delete-token">
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  سيتم حذف الرمز "{selectedToken?.label}" نهائياً. لا يمكن التراجع عن هذا الإجراء.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
