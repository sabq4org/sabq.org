...
function TodayGroupTitle({ title, count }: { title: string; count: number }) {
  return (
    <div className="flex items-center justify-between gap-2 px-1 pt-1 text-[11px] font-black text-foreground">
      <span>{title}</span>
...
function TodayMatchesBoard({ items, onOpen }: { items: SpLiveItem[]; onOpen: (id: number) => void }) {
  ...
  const worldCup = items.filter((f) => f.competitionSlug === "world-cup");
  const others = items.filter((f) => f.competitionSlug !== "world-cup");
  ...
  return (
    <div className="border-b border-border bg-card">
      <div className="max-w-6xl mx-auto px-4 py-3.5">
        ...
        {/* الجوال: مجموعات مدمجة؛ كأس العالم أولًا ثم بقية البطولات. */}
        <div className="sm:hidden space-y-3">
          {groups.map((group) => (
            <div key={group.key} className="grid gap-2">
              <TodayGroupTitle title={group.title} count={group.items.length} />
              {group.items.map((f) => <TodayCompactRow key={f.id} f={f} onOpen={onOpen} />)}
            </div>
          ))}
        </div>
        {/* الديسكتوب: مجموعات أفقية مختصرة؛ لا نخلط كأس العالم مع الدوريات الأخرى. */}
        <div className="hidden sm:block space-y-3">
          {groups.map((group) => (
            <div key={group.key} className="space-y-2">
              <TodayGroupTitle title={group.title} count={group.items.length} />
              <div className="flex gap-3 overflow-x-auto pb-1 -mx-4 px-4 snap-x scrollbar-hide">
                {group.items.map((f) => <TodayMiniCard key={f.id} f={f} onOpen={onOpen} />)}
              </div>
            </div>
          ))}
        </div>
        <Link href="/sports/matches" className={`mt-2 inline-flex items-center gap-1 text-xs font-black ${ACCENT} hover:underline`}>
...
export default function SportsHub() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<SpCompetitionCategory>("saudi");
  const [autoPickCategory, setAutoPickCategory] = useState<SpCompetitionCategory | null>("saudi");
  ...
  const activeCat: SpCompetitionCategory = selectedCategory;
  ...
  const rankCompetitionInCategory = (c: SpCompetition): number => {
    if (liveCompSlugs.has(c.slug)) return 0;
    if (todayCompSlugs.has(c.slug)) return 1;
    return 2 + COMP_STATUS_RANK[c.status ?? "unknown"];
  };

  // داخل الفئة: البطولات التي لديها مباشر الآن أولًا، ثم مباريات اليوم،
  // ثم حسب حالة الموسم، مع الحفاظ على الترتيب الأصلي عند التعادل.
  const sortedCompsForCat = (cat: SpCompetitionCategory): SpCompetition[] => competitions
    .filter((c) => catOf(c) === cat)
    .map((c, i) => ({ c, i }))
    .sort((a, b) => {
      const ra = rankCompetitionInCategory(a.c);
      const rb = rankCompetitionInCategory(b.c);
      return ra !== rb ? ra - rb : a.i - b.i;
    })
    .map(({ c }) => c);

  const compsInActiveCat = sortedCompsForCat(activeCat);

  // عند اختيار فئة نختار أول بطولة بنفس ترتيب الرقائق المعروض، حتى لا تظهر
  // رقاقة أولى ومحتوى بطولة مختلفة.
  const pickCompForCat = (cat: SpCompetitionCategory): string | null => {
    return sortedCompsForCat(cat)[0]?.slug ?? null;
  };

  // عند دخول فئة جديدة نثبّت المحتوى على أول رقاقة ظاهرة. إذا وصلت بيانات
  // "مباريات اليوم" بعد الضغط، يعاد الاختيار مرة واحدة حسب الأولوية الأحدث.
  useEffect(() => {
    if (!autoPickCategory || autoPickCategory !== activeCat) return;
    const slug = pickCompForCat(autoPickCategory);
    if (slug && slug !== compSlug) setCompSlug(slug);
  }, [autoPickCategory, activeCat, compSlug, competitions, todayMatches]);
                ...
                <div className="flex gap-2 overflow-x-auto pb-2">
                  {compsInActiveCat.map((c) => (
                    <button key={c.slug} onClick={() => {
                      setSelectedCategory(catOf(c));
                      setAutoPickCategory(null);
                      setCompSlug(c.slug);
                    }}
