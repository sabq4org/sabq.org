...
import { ArrowRight, Award, Bandage, BarChart3, CalendarRange, Landmark, ListOrdered, MapPin, Shirt, UserCog, Users, Wallet } from "lucide-react";
...
import { AcMatchCard } from "@/components/asiancup/AcMatchCard";
...
import {
  ...
  type AcStandingRow,
  type AcTeamProfile,
} from "@/components/asiancup/acTypes";
...
function GroupRow({ row, currentTeamId }: { row: AcStandingRow; currentTeamId: number }) {
  ...
  return (
    <Link href={`/asian-cup/team/${row.team.id}`} className={`${className} hover-elevate active-elevate-2 transition-all`}>
      {inner}
    </Link>
  );
}
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
        {fixtures.map((fixture, index) => (
          ...
          >
            <AcMatchCard fixture={fixture} onOpen={onOpenMatch} />
          </motion.div>
        ))}
      </div>
    </div>
...
function SquadPlayerButton({
  ...
  return (
    <button
      ...
      data-testid={`ac-player-${player.id}`}
    >
...
export default function AsianCupTeam() {
  const { user } = useAuth();
  ...
  const { data, isLoading, isError } = useQuery<AcTeamProfile>({
    queryKey: [`/api/asian-cup/team/${teamId}`],
    enabled: validId,
    staleTime: 60 * 1000,
  ...
  useEffect(() => {
    document.title = data?.team
      ? `${data.team.name} — كأس آسيا 2027 | سبق`
      : "المنتخب — كأس آسيا 2027 | سبق";
  }, [data?.team]);
              ...
              <div className="flex items-center gap-5">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-3 flex-wrap mt-2 text-sm text-emerald-50/90">
                    {(data.group?.name || data.stats.groupName) && (
              ...
              <section className="py-8 border-b border-border/60">
                <div className="container max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h2 className="text-xl font-bold">{data.group.name}</h2>
                  </div>
