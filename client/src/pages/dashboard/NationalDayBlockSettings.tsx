...
import { AlertCircle, Flag, Plus, X, Save, Eye } from "lucide-react";
import { NationalDay96Block } from "@/components/seasonal/NationalDay96Block";

/**
 * صفحة إدارة بلوك «اليوم الوطني الـ96» — نمط صفحة بلوك الحج نفسه:
 * تفعيل، عنوان، كلمات الاكتشاف التلقائي، سقف العدد، النافذة الزمنية،
 * تواريخ الموسم، والأخبار المثبّتة. المعاينة المباشرة تعرض نفس المكوّن
 * الذي تعرضه الرئيسية (بالإعدادات المحفوظة على السيرفر).
 */
export default function NationalDayBlockSettings() {
  const { user } = useAuth({ redirectToLogin: true });
  const { toast } = useToast();

  const allowed = hasRole(user, "admin", "system_admin");

  const settings = useQuery<{ config: NDConfig | null }>({
    queryKey: ["/api/national-day-block/admin"],
    enabled: !!user && allowed,
  });

  const [form, setForm] = useState<NDConfig>(emptyConfig());
  ...
  const save = useMutation({
    mutationFn: async (payload: Partial<NDConfig>) =>
      apiRequest("/api/national-day-block/admin", {
        method: "PUT",
        body: JSON.stringify(payload),
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/national-day-block/admin"] });
      queryClient.invalidateQueries({ queryKey: ["/api/national-day-block"] });
      ...
      <DashboardLayout>
        <div className="p-8 text-center" dir="rtl">
          ...
          <p className="text-muted-foreground">
            صفحة إعدادات بلوك اليوم الوطني متاحة لحسابات الإدارة.
          </p>
        </div>
      </DashboardLayout>
    );
  ...
  return (
    <DashboardLayout>
      <div className="mx-auto w-full max-w-[1600px] space-y-6 px-4 pb-10 sm:px-6" dir="rtl">
        <DashboardPageHeader
          icon={Flag}
          title="بلوك اليوم الوطني الـ96"
          description={'«عزّنا بطبعنا» — يظهر في الصفحة الرئيسية خلال موسم اليوم الوطني'}
        />
            ...
            <div className="space-y-5">
                  ...
                  <div className="flex items-center justify-between p-3 rounded-lg bg-muted/40">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        عند التفعيل، يظهر البلوك في الرئيسية ضمن نطاق التاريخ
                      </p>
                    </div>
                    <Switch
                      checked={form.isActive}
                      onCheckedChange={(v) => setForm({ ...form, isActive: v })}
                      data-testid="switch-nd96-active"
                    />
                  </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  <p className="text-xs text-muted-foreground">
                    تُستخدم لاكتشاف أخبار اليوم الوطني تلقائيًا. أي مقال يحتوي
                    إحدى هذه الكلمات في عنوانه أو ملخصه يظهر في البلوك.
                  </p>
                  ...
                  <div className="flex gap-2">
                    <Input
                      ...
                      maxLength={50}
                      data-testid="input-nd96-new-keyword"
                    />
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setForm({ ...form, articleLimit: Math.max(1, Math.min(12, Number(e.target.value) || 3)) })
                      }
                      data-testid="input-nd96-limit"
                    />
                    <p className="text-xs text-muted-foreground mt-1">
                      التصميم يعرض 3 بطاقات في الصف الواحد
                    </p>
                  </div>
                  <div>
                    <label className="text-sm font-medium mb-1.5 block">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setForm({ ...form, lookbackHours: Math.max(1, Math.min(720, Number(e.target.value) || 72)) })
                      }
                      data-testid="input-nd96-lookback"
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        data-testid="input-nd96-end"
                      />
                    </div>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    المقترح: من 16 سبتمبر (أسبوع القيم) حتى 26 سبتمبر 2026.
                    سمة الهيدر مستقلة وتعمل تلقائيًا 21–25 سبتمبر.
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  <p className="text-xs text-muted-foreground">
                    تتصدر البلوك دائمًا قبل الأخبار المُكتشفة من الكلمات المفتاحية.
                  ...
                  <div className="flex gap-2">
                    <Input
                      ...
                      data-testid="input-nd96-new-pinned"
                    />
              ...
              <Card>
                ...
                <CardContent>
                  <p className="text-xs text-muted-foreground mb-3">
                    ...
                    بعد التحرير اضغط «حفظ التغييرات» لرؤية التحديث. إذا لم يظهر
                    شيء فالبلوك مخفي حاليًا (معطّل، خارج النافذة، أو لا توجد
                    أخبار مطابقة).
                  </p>
                  <NationalDay96Block />
                </CardContent>
